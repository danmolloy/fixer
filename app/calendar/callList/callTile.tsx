...
export default function CallTile(props: CallTileProps) {
  ...
  return (
    <Link
      href={`/event/${eventCall.event.id}`}
      data-testid={`${eventCall.id}-call-tile`}
      className='m-2 flex w-full flex-col rounded border-b p-2 hover:bg-slate-50 lg:w-[50vw]'
    >
      ...
      <div className='flex flex-row items-center text-gray-700'>
        <AiOutlineCalendar />
        <p className='ml-1'>
          {DateTime.fromJSDate(new Date(eventCall.startTime)).toFormat(
            'hh:mm a'
          )}
        </p>
      </div>
