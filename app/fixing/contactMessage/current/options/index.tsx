...
export default function CurrentContactsOptions(props: CurrentContactsOptionsProps) {
  const { contact, index, numContacts, eventCalls } = props;
  const [position, setPosition] = useState<string>(contact.position)
  const router = useRouter();

  const handleDelete = async () => {
    const conf = confirm(`Are you sure you want to remove this player from the list?`)
    if (conf) {
      return await axios.post("/fixing/contactMessage/api/delete", {contactMessageId: contact.id}).then(() => {
        router.refresh()
      })
    }
  }
  ...
  const handleUpdate = async (data: {}, confText: string) => {
    if (confirm(confText)) {
      return await axios.post("/fixing/contactMessage/api/update", {id: contact.id, data: data}).then(() => {
        router.refresh()
      })
    }
  }
  ...
  const handleUpdateMessage = async() => {
    ...
    if (message) {
      return await axios.post("/fixing/contactMessage/api/update", {id: contact.id, data: {playerMessage: message}}).then(() => {
        router.refresh()
      })
    }
  }
  ...
  const handleShift = async(movingUp: boolean) => {
    ...
    if (confirm(confText)) {
      return await axios.post("/fixing/contactMessage/api/update/shift", {
        eventSectionId: contact.eventSectionId, 
        movingUp: movingUp, 
        contactMessageId: contact.id}).then(() => {
        router.refresh()
      })
    }
  }
  ...
  return (
    <div data-testid="contact-options" className="">
      ...
      <button 
        ...
        onClick={() => handleUpdate({accepted: true}, "Are you sure you want to accept the work on this players behalf?")}>
        Accept
      </button>
      <button
        className="disabled:opacity-40"         
        disabled={contact.accepted === false }
        onClick={() => handleUpdate({accepted: false}, "Are you sure you want to decline the work on this players behalf?")}>
        Decline
      </button>
      ...
      <div>
        <select data-testid="position-select" onChange={(e) => setPosition(e.target.value)} value={position}>
          ...
          <button onClick={() => handleUpdate({position: position}, `Are you sure you want to update this player's position to ${position}?`)}>
            Update Position
          </button>
      </div>
