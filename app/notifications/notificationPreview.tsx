...
export default function NotificationPreview(props: NotificationPreviewProps) {
  ...
  return (
    <Link
      href={`/notifications/${playerCall.id}`}
      data-testid={`${playerCall.id}-preview`}
      className={`${playerCall.accepted === false || (endDate < DateTime.now().startOf('day') && 'opacity-30')} my-1 w-full rounded p-2 shadow hover:bg-indigo-100`}
    >
      ...
      <div className='flex flex-row text-sm text-gray-600'>
        ...
        <p className='ml-2'>
          recieved{' '}
          {DateTime.fromJSDate(new Date(playerCall.recievedDate!)).toFormat(
            'HH:mm DD'
          )}
        </p>
      </div>
        ...
        <div className='flex flex-row items-center'>
          ...
          <p>
            {startDate.hasSame(endDate, 'day')
              ...
              : `${startDate.toFormat('dd LLL yyyy')} - ${endDate.toFormat('dd LLL yyyy')}`}
          </p>
        </div>
      </div>
    </Link>
