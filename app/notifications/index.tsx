...
export default function Notifications(props: NotificationsProps) {
  ...
  const [playerCallFilter, setPlayerCallFilter] = useState<
    'action' | 'past' | null
  >(null);
  ...
  const ensembleSet = () => {
    ...
    for (let i = 0; i < playerCalls.length; i++) {
      if (
        arr.filter(
          (j) => j.id === playerCalls[i].eventSection.event.ensemble.id
        ).length === 0
      ) {
  ...
  return (
    <div
      data-testid='notifications-index'
      className='flex w-full flex-col md:flex-row'
    >
      <NotificationsDashboard
        setEnsembleFilter={(ensemble) => {
          ensemble === ensembleFilter
            ? setEnsembleFilter(null)
            : setEnsembleFilter(ensemble);
        }}
        ensembleFilter={ensembleFilter}
        playerCallFilter={playerCallFilter}
        setPlayerCallFilter={(filter) => {
          filter === playerCallFilter
            ? setPlayerCallFilter(null)
            : setPlayerCallFilter(filter);
        }}
