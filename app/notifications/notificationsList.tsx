...
export default function NotificationsList(props: NotificationsListProps) {
  ...
  const notificationsFilter = (playerCall: PlayerCallNotification) => {
    if (playerCallFilter === "action") {
      return playerCall.accepted === null 
      ? true : false
  ...
  return (
    <div data-testid="notifications-list" className=" p-4 flex flex-col items-center justify-center w-[95vw] md:w-3/5">
      ...
      : ensembleFilter !== null 
      ? playerCalls.filter(i => i.eventSection.event.ensembleId === ensembleFilter).map(i => (
        <NotificationPreview key={i.id} playerCall={i} />
      ))
      : playerCalls.filter(i => notificationsFilter(i)).map(i => (
        <NotificationPreview key={i.id} playerCall={i} />
      ))}
    </div>
  )
