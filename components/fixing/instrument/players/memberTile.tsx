...
export default function MemberTile(props: MemberTileProps) {
  const { disabled, member, setSelectMember } = props
  return (
    <div data-testid={`${member.id}-member-tile`} className={`${disabled && "opacity-30"} flex flex-row border-b items-center justify-between w-full p-1`}>
      ...
      : <div className="flex flex-col">
        ...
        <div>
          ...
          <button
            data-testid={`${member.user.id}-select-btn`}
            disabled={disabled} 
