...
const mockProps = {
  // This obj isn't typed because I pass propsValue to the component directly.
  name: 'venue',
  id: 'qwertyId',
  label: 'mockLabel',
  ...
  beforeEach(() => {
    render(
      <Formik
        ...
        onSubmit={() => {}}
      >
        {(props) => <ComboBox propsValue={props.values.venue} {...mockProps} />}
      </Formik>
    );
  });
    ...
    for (let i = 0; i < mockProps.options.length; i++) {
      await act(async () => {
        await fireEvent.change(textInput, {
          target: { value: mockProps.options[i].textPrimary.slice(0, 3) },
        });
      });
      ...
      expect(mockProps.setValue).toHaveBeenCalledWith(
        mockProps.options[i].textPrimary
      );
    }
  });
  it('clicking on option completes text entry and sets value', async () => {
    ...
    for (let i = 0; i < mockProps.options.length; i++) {
      ...
      const randInd = Math.floor(
        Math.random() * (mockProps.options[i].textPrimary.length - 3)
      );
      await act(async () => {
        await fireEvent.change(textInput, {
          target: {
            value: mockProps.options[i].textPrimary.slice(randInd, randInd + 3),
          },
        });
      });
      expect(optionsDiv).toBeInTheDocument();
