...
import CreateEventForm, { CreateEventFormProps, formatDate} from "../../../../app/event/create/form"
...
describe("<CreateEventForm />", () => {
  ...
  it("'Organisation' select is in the document with label, blank option & all options with label & value", () => {
    ...
    for (let i = 0; i < mockProps.adminEnsembleList.length; i ++) {
      expect(orgSelect.textContent).toMatch(mockProps.adminEnsembleList[i].ensemble.name)
      const option = screen.getByText(mockProps.adminEnsembleList[i].ensemble.name)
      expect(option).toHaveAttribute("value", mockProps.adminEnsembleList[i].ensembleId)
    }
  })
  ...
  it("all err messages are in the document on submit btn click", async () => {
    ...
    expect(createForm.textContent).toMatch("Please revise your form. Errors are stated in red.")
    expect(createForm.textContent).toMatch("Event confirmation status required")
  ...
  it("if all fields correct, axios.post() & useRouter() are called with expected args", async () => {
    ...
    const concertProgram = screen.getByLabelText("Concert Program")
    await act(async () => {
      fireEvent.change(concertProgram, {target: { value: mockVals.concertProgram}})
    })
    const dressCode = screen.getByLabelText("Dress Code")
    await act(async () => {
    ...
    const additionalInfo = screen.getByLabelText("Additional Information")
    await act(async () => {
      fireEvent.change(additionalInfo, {target: { value: mockVals.additionalInfo}})
    })
    const callStart = screen.getByTestId(`calls.0.startTime`)
    await act(async () => {
      fireEvent.change(callStart, {target: { value: formatDate(mockVals.calls[0].startTime)}})
    })
    const callEnd = screen.getByTestId(`calls.0.endTime`)
    await act(async () => {
      fireEvent.change(callEnd, {target: { value: formatDate(mockVals.calls[0].endTime)}})
    })
...
describe("<CreateEventForm />", () => {
  ...
  it("Ensemble name radio group is in the document with label, all options have label, name, value & type attrs", () => {
    const ensembleNames = mockProps.adminEnsembleList.find(i => i.ensembleId === mockEnsembleAdmin.ensembleId)?.ensemble.ensembleNames
