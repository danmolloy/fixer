...
import SentEmailList, { SentEmailListProps } from "../../../../../app/event/[id]/messages/sentList";
...
describe("<SentEmailList", () => {
  ...
  it("all emails previews are in the document with email, subject, status and date", () => {
    for (let i = 0; i < mockProps.emails.length; i ++) {
      const emailPreview = screen.getByTestId(`${mockProps.emails[i].id}-preview`)
      ...
      expect(emailPreview.textContent).toMatch(DateTime.fromJSDate(mockProps.emails[i].timestamp).toFormat("f"));
    }
  })
