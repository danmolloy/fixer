...
import MessagesHeader, {
  MessagesHeaderProps,
} from '../../../../../app/event/[id]/messages/header';
...
describe('<MessagesHeader />', () => {
  ...
  it('ensemble name and date range is in the document', () => {
    const identifier = screen.getByText(
      `${mockProps.event.ensembleName} ${getDateRange(mockProps.event.calls)}`
    );
  ...
  it('sent emails count is in the document', () => {
    const emailsCount = screen.getByText(
      `${mockProps.event.sentEmails.length} sent email(s)`
    );
