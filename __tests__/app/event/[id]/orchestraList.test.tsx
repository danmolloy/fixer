  ...
  it('booked musicians names are in respective sections, with TBC for all empty seats', () => {
    for (let i = 0; i < mockProps.sections.length; i++) {
      ...
      expect(section.textContent).toMatch(
        mockProps.sections[i].ensembleSection.name
      );
      const acceptedContacts = mockProps.sections[i].contacts.filter(
        (c) => c.accepted === true
      );
      ...
      const numEmptySeats =
        mockProps.sections[i].numToBook -
        mockProps.sections[i].contacts.filter((c) => c.accepted === true)
          .length;
      const emptySeats = screen.getAllByTestId(
        `${mockProps.sections[i].id}-tbc`
      );
