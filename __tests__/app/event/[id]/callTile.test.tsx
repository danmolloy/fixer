...
describe("CallTile component", () => {
  ...
  it("startTime is in the document", () => {
    const callTile = screen.getByTestId("call-tile-div")
    expect(callTile.textContent).toMatch(String(DateTime.fromJSDate(new Date(mockProps.startTime)).toFormat("HH:mm DD")))
    expect(callTile.textContent).toMatch("11:59 Jul 15, 2024")
  })
  it("endTime is in the document", () => {
    const callTile = screen.getByTestId("call-tile-div")
    expect(callTile.textContent).toMatch(String(DateTime.fromJSDate( new Date(mockProps.endTime)).toFormat("HH:mm DD")))
