...
import OrchestrationSummary, { OrchestrationSummaryProps } from "../../../../app/fixing/eventSection/orchestration"
...
describe("<OrchestrationSummary />", () => {
  ...
  it("each call has formatted date & (numRequired or 0)", () => {
    for (let i = 0; i < mockProps.eventCalls.length; i ++) {
      const numRequired = mockProps.orchestration.find(orch => orch.callId === mockProps.eventCalls[i].id)?.numRequired || 0
