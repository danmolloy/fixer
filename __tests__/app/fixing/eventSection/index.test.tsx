...
import EventSectionIndex, { EventSectionProps } from "../../../../app/fixing/eventSection";
...
describe("<EventSectionIndex />", () => {
  ...
  it("<EventSectionIndex /> renders", () => {
    const eventSection = screen.getByTestId(`${mockProps.section.id}-event-section`)
  ...
  it("number of players to book is stated", () => {
    const numToBook = screen.getByText(`Booking ${mockProps.section.numToBook} player(s)`)
  ...
  it("delete btn is in the document and calls global.confirm() & axios.post() on click", () => {
    ...
    expect(mockConfirm).toHaveBeenCalledWith('Are you sure you want to delete this section?')
    expect(axios.post).toHaveBeenCalledWith("/fixing/eventSection/api/delete", {sectionId: mockProps.section.id})
  })
