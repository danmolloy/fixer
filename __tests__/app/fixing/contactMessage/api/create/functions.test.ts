...
import { CreateContactMessageProps, createContactMessages, generateToken, getDateRange, getNumToContact, gigIsFixed, } from '../../../../../../app/fixing/contactMessage/api/create/functions';
...
jest.mock('../../../../../../app/fixing/contactMessage/api/create/emailFunctions', () => ({
...
describe("createContactMessages", () => {
  ...
  it("fetches correct contactMessage data", async () => {
    prismaMock.contactMessage.findMany.mockResolvedValueOnce([mockContactMessage]);
  ...
  it("gives new all contactMessages correct vals incl. index numbers", async () => {
    ...
    expect(prismaMock.contactMessage.create).toHaveBeenCalledWith({
      data:{eventSectionId: Number(mockContactMessages.eventSectionId),
...
describe("numToContact", () => {
  it("returns correct number of musicians to contact", () => {
    ...
    expect(getNumToContact({contactMessages: contacts, numToBook: 12})).toBe(9);
  });
});
