...
import UpdateContactMessage, { UpdateContactMessageProps } from "../../../../../app/fixing/contactMessage/update/form"
...
describe("<UpdateContactMessage />", () => {
  ...
  it("Contact full name & instrument are in the document", () => {
    const contact = screen.getByText(`${mockProps.contact.contact.firstName} ${mockProps.contact.contact.lastName} (${mockProps.instrument})`);
  ...
  it("All calls status select is in the document with formatted date label & 7 options", () => {
    for (let i = 0; i < mockProps.contact.eventCalls.length; i ++) {
      ...
      expect(call.textContent).toMatch(DateTime.fromJSDate(new Date(mockProps.contact.eventCalls[i].call.startTime)).toFormat('HH:mm'))
      expect(call.textContent).toMatch(DateTime.fromJSDate(new Date(mockProps.contact.eventCalls[i].call.startTime)).toFormat('DD'))
      const statusSelect = screen.getByTestId(`${mockProps.contact.eventCalls[i].callId}-select`)
