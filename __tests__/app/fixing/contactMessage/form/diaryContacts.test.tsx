...
import DiaryContacts, {
  DiaryContactsProps,
} from '../../../../../app/fixing/contactMessage/form/diaryContacts';
  ...
  it("all sectionContacts who aren't archived are in the document", () => {
    for (let i = 0; i < mockProps.sectionContacts.length; i++) {
      ...
      if (mockProps.sectionContacts[i].status === 'ARCHIVED') {
        expect(diaryContacts.textContent).not.toMatch(
          `${mockProps.sectionContacts[i].firstName} ${mockProps.sectionContacts[i].lastName}`
        );
      } else {
        expect(diaryContacts.textContent).toMatch(
          `${mockProps.sectionContacts[i].firstName} ${mockProps.sectionContacts[i].lastName}`
        );
      }
    }
...
describe('<DiaryContacts />', () => {
  ...
  it('if !sectionContacts, there is a help message', () => {
    ...
    expect(helpMsg.textContent).toMatch(
      'Remove your filters, or add new contacts to you Address Book.'
    );
  });
});
