...
import AppendedContacts, {
  AppendedContactsProps,
} from '../../../../../../app/fixing/contactMessage/form/appendedContacts';
...
describe('<AppendedContacts />', () => {
  ...
  it('all event calls have a column with formatted date label', () => {
    for (let i = 0; i < mockProps.eventCalls.length; i++) {
      ...
      expect(callCol.textContent).toMatch(
        DateTime.fromJSDate(
...
describe('<AppendedContacts />', () => {
  ...
  it('if !contacts, there is a help message', () => {
    ...
    expect(appendedContacts.textContent).toMatch(
      'Select from your diary contacts below.'
    );
  });
});
