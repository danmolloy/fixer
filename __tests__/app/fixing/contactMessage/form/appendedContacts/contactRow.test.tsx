...
const mockProps: AppendedContactRowProps = {
  contact: {
    ...
    calls: [mockCall.id],
    autoAccepted: Math.random() > .5 ? true : false
  },
  type: Math.random() > .6 ? "AUTOBOOK" : Math.random() > .3 ? "BOOKING" : "AVAILABILITY",
  index: 0,
  eventCalls: [mockCall],
...
describe('<AppendedContactRow />', () => {
  ...
  it('position text input is in the document with expected initial val', () => {
    ...
    expect(positionInput).toHaveAttribute("name", `contacts[${mockProps.index}]position`);
  ...
  it('all event call checkboxes are in the document with checked, type, value & name attrs', () => {
    for (let i = 0; i < mockProps.eventCalls.length; i ++) {
      const eventCall = screen.getByTestId(`${mockProps.eventCalls[i].id}-field`);
      expect(eventCall).toBeInTheDocument();
      expect(eventCall).toHaveAttribute("type", "checkbox");
      expect(eventCall).toHaveAttribute("name", `contacts[${mockProps.index}]calls`);
      //expect(eventCall).toHaveFormValues(Number(mockProps.eventCalls[i].id));
      if (mockProps.contact.calls.map(j => Number(j)).includes(Number(mockProps.eventCalls[i].id))) {
...
describe("<AppendedContactRow /> Options", () => {
  ...
  it("moveUp btn is in the menu calls 'swap(index, index - 1)' on click", () => {
    ...
    expect(mockProps.swap).toHaveBeenCalledWith(mockProps.index, mockProps.index - 1);
  });
  it("add msg btn is in the menu and calls handleAddMessage() on click", () => {
    ...
    expect(global.prompt).toHaveBeenCalledWith("What message would you like to add to this player?");
    expect(mockProps.addPlayerMessage).toHaveBeenCalledWith(mockProps.index, "mock prompt text");
  })
...
describe('<AppendedContactRow />', () => {
  const mockProps: AppendedContactRowProps = {
    ...
    type: Math.random() > .5 ? "AUTOBOOK" : "BOOKING",
  ...
  it("if !AVAILABILITY, queue number is in the document", () => {
    ...
    expect(queueNum.textContent).toMatch(String(mockProps.currentCallCount + mockProps.index + 1));
  })
  it("if !AVAILABILITY, autoAccept checkbox is in the document with expected attrs", () => {
    ...
    expect(autoAccept).toHaveAttribute("name", `contacts[${mockProps.index}]autoAccepted`)
