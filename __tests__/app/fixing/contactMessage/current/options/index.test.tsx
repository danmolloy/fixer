...
import CurrentContactsOptions, { CurrentContactsOptionsProps } from "../../../../../../app/fixing/contactMessage/current/options";
...
describe("<CurrentContactsOptions />", () => {
  ...
  it("if availability check, offer to book btn is in the document & calls confirm, axios and useRouter on click", async () => {
    ...
    expect(confirmMock).toHaveBeenCalledWith("Are you sure you want to reoffer as a booking?")
    expect(axios.post).toHaveBeenCalledWith("/fixing/contactMessage/api/update", {
  ...
  it("decline btn is in the document and calls confirm, axios.post and useRouter on click", async () => {
    ...
    expect(confirmMock).toHaveBeenCalledWith("Are you sure you want to decline the work on this players behalf?")
    expect(axios.post).toHaveBeenCalledWith("/fixing/contactMessage/api/update", {
      id: mockProps.contact.id, 
      data: {
        accepted: false
  ...
  it("update player message is in the document and calls prompt, axios.post and useRouter with expected args on click", async () => {
    ...
    expect(promptMock).toHaveBeenCalledWith("What message would you like to add to this player?")
    expect(axios.post).toHaveBeenCalledWith("/fixing/contactMessage/api/update", {
      id: mockProps.contact.id, 
  ...
  it("move up btn is in the document and calls handleShift(true) on click", async () => {
    ...
    expect(confirmMock).toHaveBeenCalledWith(`Are you sure you want to move this player up one list position?`)
    expect(axios.post).toHaveBeenCalledWith("/fixing/contactMessage/api/update/shift", {
  ...
  it("move down btn is in the document and calls handleShift(false) on click", async () => {
    ...
    expect(confirmMock).toHaveBeenCalledWith(`Are you sure you want to move this player down one list position?`)
    expect(axios.post).toHaveBeenCalledWith("/fixing/contactMessage/api/update/shift", {
        eventSectionId: mockProps.contact.eventSectionId, 
        movingUp: false, 
        contactMessageId: mockProps.contact.id})
  ...
  it("delete btn is in the document and calls prompt, axios & useRouter with expected args on click", async () => {
    ...
    expect(confirmMock).toHaveBeenCalledWith("Are you sure you want to remove this player from the list?")
    expect(axios.post).toHaveBeenCalledWith("/fixing/contactMessage/api/delete", 
      { contactMessageId: mockProps.contact.id }
    )
  })
  ...
  it("update postion btn is in the document and calls prompt, axios.post and useRouter() on click", async () => {
    ...
    expect(confirmMock).toHaveBeenCalledWith(`Are you sure you want to update this player's position to ${mockProps.contact.position}?`)
    expect(axios.post).toHaveBeenCalledWith("/fixing/contactMessage/api/update", {
      id: mockProps.contact.id, 
  ...
  it("<UpdateContactEventCalls /> is in the document with expected props", () => {
    ...
    for (let i = 0; i < mockProps.eventCalls.length; i ++) {
      const eventCall = screen.getByLabelText(`${DateTime.fromJSDate(new Date(mockProps.eventCalls[i].startTime)).toFormat("HH:mm")}${DateTime.fromJSDate(new Date(mockProps.eventCalls[i].startTime)).toFormat("DD")}`)
...
describe("<CurrentContactsOptions />", () => {
  ...
  it("accept btn is in the document and calls confirm, axios and useRouter with expected args on click", async () => {
    ...
    expect(confirmMock).toHaveBeenCalledWith("Are you sure you want to accept the work on this players behalf?")
    expect(axios.post).toHaveBeenCalledWith("/fixing/contactMessage/api/update", {
      id: mockProps.contact.id, 
      data: {
        accepted: true
