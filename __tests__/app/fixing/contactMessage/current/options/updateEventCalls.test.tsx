...
import UpdateContactEventCalls, { UpdateContactEventCallsProps } from "../../../../../../app/fixing/contactMessage/current/options/updateEventCalls";
...
describe("<UpdateContactEventCalls", () => {
  ...
  it("<UpdateContactEventCalls /> renders", () => {
    ...
    expect(updateEventCalls.textContent).not.toMatch("At least one call must be offered.")
  })
  it("all event calls are in the document with label, value, name attrs, & checked if appropriate", () => {
    for (let i = 0; i < mockProps.eventCalls.length; i ++) {
      ...
      expect(eventCall).toHaveAttribute("value", String(mockProps.eventCalls[i].id))
      expect(eventCall).toHaveAttribute("name", "calls")
      if (mockProps.contact.calls.map(j => String(j)).includes(String(mockProps.eventCalls[i].id))) {
  ...
  it("at least one call must be offered or not be removed", async () => {
    ...
    expect(global.alert).toHaveBeenCalledWith("This player will no longer have any calls offered, which is not a valid action. If you wish to remove them from the event, delete them from the list.")
  })
...
describe("<UpdateContactEventCalls", () => {
  ...
  const eventCallOne = screen.getByLabelText(`${DateTime.fromJSDate(new Date(mockProps.eventCalls[1].startTime)).toFormat("HH:mm")}${DateTime.fromJSDate(new Date(mockProps.eventCalls[1].startTime)).toFormat("DD")}`)
...
describe("<UpdateContactEventCalls", () => {
  ...
  it("at least one call must be offered", async () => {
    ...
    expect(axios.post).not.toHaveBeenCalled()
    expect(updateEventCalls.textContent).toMatch("At least one call must be offered.")
  })
  it("Update Calls btn is in the document and calls axios.post and useRouter on click", async () => {
    const updateBtn = screen.getByText("Update Calls")
    const updateEventCalls = screen.getByTestId("update-event-calls")
    const eventCallZero = screen.getByLabelText(`${DateTime.fromJSDate(new Date(mockProps.eventCalls[0].startTime)).toFormat("HH:mm")}${DateTime.fromJSDate(new Date(mockProps.eventCalls[0].startTime)).toFormat("DD")}`)
