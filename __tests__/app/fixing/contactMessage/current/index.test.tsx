import CurrentContactMessages, { CurrentContactMessagesProps } from "../../../../../app/fixing/contactMessage/current";
...
describe("<CurrentContactMessages />", () => {
  const mockProps: CurrentContactMessagesProps = {
    ...
    contacts: [
      {...mockContactMessage,
      contact: {...mockEnsembleContact, firstName: "Bob", lastName: "Jeffrey"},
      id: 1,
      type: "AUTOBOOK",
      calls: [mockCall]},
      {...mockContactMessage,
        contact: {...mockEnsembleContact, firstName: "Lachlan", lastName: "Johnson"},
        id: 2,
        type: "AVAILABILITY",
        calls: [mockCall]},
        {...mockContactMessage,
          contact: {...mockEnsembleContact, firstName: "Amy", lastName: "Brookman"},
          id: 3,
          type: "BOOKING",
          calls: [mockCall]},
    ],
    type: Math.random()  > .5 ? "AVAILABILITY" : "BOOKING"
  }
  beforeEach(() => {
  ...
  it("All event calls with formatted date column heads are in the document", () => {
    for (let i = 0; i < mockProps.eventCalls.length; i ++) {
      ...
      const startTime = DateTime.fromJSDate(new Date(mockProps.eventCalls[i].startTime)).toFormat("HH:mm");
      const startDate = DateTime.fromJSDate(new Date(mockProps.eventCalls[i].startTime)).toFormat("DD");
