...
import ResponseConf, { ResponseConfProps } from "../../../../../app/fixing/response/[token]/responseConf";
...
describe("<ResponseConf />", () => {
  ...
  it("if ERROR, states to contact GigFix", () => {
    const responseConf = screen.getByTestId("response-confirmation");
    expect(responseConf.textContent).toMatch("There has been an error. Please contact GigFix immediately.");
  })
})
...
describe("<ResponseConf />", () => {
  ...
  contactMessage: {
    ...mockContactMessage,
    type: Math.random() > .5 ? "BOOKING" : "AUTOBOOK",
    status: "FINDINGDEP",
    calls: [mockCall],
    eventSection: {
      ...mockEventSection,
  ...
  it("if FINDINGDEP, states that we are looking & player is still booked", () => {
    ...
    const responseConf = screen.getByTestId("response-confirmation");
    expect(responseConf.textContent).toMatch("You are currently booked for this work, however we are trying to find you a dep.")
  })
})
...
describe("<ResponseConf /> AVAILABILITY", () => {
  ...
  it("if AVAILABILITY, indicates if availability was successfully given with list of calls", () => {
    ...
    expect(header).toHaveRole("heading");
    const responseConf = screen.getByTestId("response-confirmation");
    const callsAvailableFor = mockProps.contactMessage.calls.filter(i => mockProps.contactMessage.availableFor.includes(i.id))

    expect(responseConf.textContent).toMatch("You are available for the following calls:")
    for (let i = 0; i < callsAvailableFor.length; i ++) {
      expect(responseConf.textContent).toMatch(
        DateTime.fromJSDate(
          new Date(callsAvailableFor[i].startTime)
        ).toFormat('HH:mm DD')
      );
    }
  })
...
describe("<ResponseConf /> AUTOBOOK", () => {
  ...
  it("if AUTOBOOK, states if you are booked", () => {
    ...
    expect(header).toHaveRole("heading");
    const responseConf = screen.getByTestId("response-confirmation");
    expect(responseConf.textContent).toMatch("The fixer automatically booked you for this work.")
  })
  it("if AUTOBOOKED, Add to Calendar btn is in the document", () => {
    ...
    expect(axios.post).toHaveBeenCalledWith("/fixing/response/ics", mockProps.contactMessage)
  })
...
describe("<ResponseConf /> AUTOBOOK", () => {
  ...
  it("if AUTOBOOK, states if you have gotten out of the work", () => {
    ...
    expect(header).toHaveRole("heading");
    const responseConf = screen.getByTestId("response-confirmation");
    expect(responseConf.textContent).toMatch("You are no longer booked for this work.")
  })
