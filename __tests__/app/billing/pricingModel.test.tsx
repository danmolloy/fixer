...
import PricingModel, { PricingModelProps } from "../../../app/billing/pricingModel";
...
describe("<PricingModel />", () => {
  ...
  it("<PricingModel /> is in the document", () => {
    const pricingModel = screen.getByTestId(`${mockProps.priceModel.id}-option`)
  ...
  it("option price and payment frequency are in the document", () => {
    ...
    expect(price.textContent).toMatch(`${mockProps.priceModel.price} ${mockProps.priceModel.paymentFrequency}`)
  ...
  it("all option features are in the document with blurb and title", () => {
    for (let i = 0; i < mockProps.priceModel.features.length; i ++) {
      const feature = screen.getByTestId(`feature-${mockProps.priceModel.features[i].id}`)
      expect(feature.textContent).toMatch(mockProps.priceModel.features[i].title)
      expect(feature.textContent).toMatch(mockProps.priceModel.features[i].blurb)
    }
  })
