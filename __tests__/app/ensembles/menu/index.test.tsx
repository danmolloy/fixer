...
import EnsemblesMenu, { EnsemblesMenuProps } from "../../../../app/ensembles/menu";
...
describe("<EnsemblesMenu />", () => {
  ...
  it("all ensembles are in the document with href", () => {
    ...
    for (let i = 0; i < mockProps.ensemblesAdmins.length; i++) {
      const ensemble = screen.getByText(mockProps.ensemblesAdmins[i].ensemble.name)
      expect(ensemble).toBeInTheDocument()
      expect(ensemble).toHaveAttribute("href", `/ensembles/${mockProps.ensemblesAdmins[i].ensemble.id}`)
    }
  })
})
