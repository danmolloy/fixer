...
import UpdateSectionForm, { UpdateSectionFormProps } from "../../../../app/ensembles/section/edit";
...
describe("<UpdateSectionForm />", () => {
  ...
  it("all contacts are in the document with indexNum, role, category, full name, up btn & down btn", async () => {
    for (let i = 0; i < mockProps.section.contacts.length; i ++) {
      const contact = screen.getByTestId(`${mockProps.section.contacts[i].id}-contact`)
      expect(contact).toBeInTheDocument()
      const indexNum = screen.getByTestId(`${mockProps.section.contacts[i].id}-indexNum`)
      expect(indexNum.textContent).toMatch(String(mockProps.section.contacts[i].indexNumber))
      expect(contact.textContent).toMatch(mockProps.section.contacts[i].role)
      expect(contact.textContent).toMatch(mockProps.section.contacts[i].category!)
      expect(contact.textContent).toMatch(`${mockProps.section.contacts[i].firstName} ${mockProps.section.contacts[i].lastName}`)
      const contactUp = screen.getByTestId(`${mockProps.section.contacts[i].id}-up`)
      const contactDown = screen.getByTestId(`${mockProps.section.contacts[i].id}-down`)
      expect(contactUp).toBeInTheDocument()
      await act(async () => {
        fireEvent.click(contactUp)
      })
      expect(indexNum.textContent).toMatch(String(mockProps.section.contacts[i].indexNumber - 1))
  ...
  it("contacts are sorted lastName < indexNum < role < category", () => {
    ...
    const sortedContacts = mockProps.section.contacts.sort((a, b) => (
