...
describe("<YearView />", () => {
  ...
  it("back toggle button calls setSelectedView with expected args", () => {
    const yearTitle = screen.getByTestId(`${mockProps.selectedDate.year}-title`)
    ...
    expect(mockProps.setSelectedDate).toHaveBeenCalledWith(mockProps.selectedDate.startOf('month').minus({years: 1}))
  ...
  it("forward toggle button calls setSelectedView with expected args", () => {
    ...
    expect(mockProps.setSelectedDate).toHaveBeenCalledWith(mockProps.selectedDate.startOf('month').plus({years: 1}))
  })
  it("all months of the year are in the document", () => {
    const janDateTime = mockProps.selectedDate.set({month: 1, year: mockProps.selectedDate.year}).startOf("month")
    for (let i = 0; i < 12; i++) {
      const monthCal = screen.getByTestId(`${janDateTime.plus({months: i}).month}-${janDateTime.year}-calendar`)
