...
describe('<MonthCalendar', () => {
  ...
  it('all expected weeks are in the document', () => {
    const startOfFirstWeek = DateTime.fromJSDate(mockProps.selectedDate)
      .startOf('month')
      .startOf('week');
    const startOfFinalWeek = DateTime.fromJSDate(mockProps.selectedDate)
      .endOf('month')
      .startOf('week');

    for (
      let i = 0;
      startOfFirstWeek.plus({ weeks: i }) < startOfFinalWeek;
      i++
    ) {
      const weekRow = screen.getByTestId(
        `week-${startOfFirstWeek.plus({ weeks: i }).weekNumber}-row`
      );
  ...
  it('events are filtered to correct days', () => {
    if (mockProps.events) {
      for (let i = 0; i < mockProps.events!.length; i++) {
        const dayOfMonth = screen.getByTestId(
          `${DateTime.fromJSDate(mockProps.events[i]!.startTime).toLocaleString()}-tile`
        );
