...
import MonthHeader, {
  MonthHeaderProps,
} from '../../../../../app/calendar/views/month/header';
...
describe('<MonthHeader />', () => {
  ...
  it('name of month is in the document', () => {
    const calendarHeader = screen.getByTestId('month-header');
    expect(calendarHeader.textContent).toMatch(
      mockProps.selectedDate.toFormat('LLLL y')
    );
  ...
  it('previous month btn is in the document and calls setSelectedDate with arg', () => {
    ...
    expect(mockProps.setSelectedDate).toHaveBeenCalledWith(
      mockProps.selectedDate.minus({ months: 1 }).startOf('month')
    );
  });
  it('next month btn is in the document and calls setSelectedDate with arg', () => {
    ...
    expect(mockProps.setSelectedDate).toHaveBeenCalledWith(
      mockProps.selectedDate.plus({ months: 1 }).startOf('month')
    );
  });
});
