...
import DayOfMonth, {
  DayOfMonthProps,
} from '../../../../../app/calendar/views/month/dayOfMonth';
...
describe('<DayOfMonth />', () => {
  ...
  it("Date is in the document with format 'd' (i.e. 13)", () => {
    ...
    expect(dayOfMonth.textContent).toMatch(
      mockProps.dayOfMonthDate.toFormat('d')
    );
  });
...
describe('<DayOfMonth />', () => {
  ...
  it('events are sorted in correct order, only first three are shown with indication of others', () => {
    ...
    expect(dayOfMonth.textContent).toMatch(
      /^131:43 AMCLM is born2:42 AMDad goes to the toilet3:42 AMDad returns from toilet& 1 more$/
    );
  });
});
