import WeekRow, {
  WeekRowProps,
  weekArr,
} from '../../../../../app/calendar/views/month/weekRow';
...
describe('<WeekRow />', () => {
  const mockProps: WeekRowProps = {
    weekStartDate: DateTime.fromJSDate(new Date('1986-10-13T01:43:00')).startOf(
      'week'
    ),
  ...
  it('<WeekRow /> is in the document without events', () => {
    const weekRow = screen.getByTestId(
      `week-${mockProps.weekStartDate.weekNumber}-row`
    );
    ...
    if (mockProps.events) {
      for (let i = 0; i < mockProps.events!.length; i++) {
        const dayOfMonth = screen.getByTestId(
          `${mockProps.events[i]?.startTime.toLocaleString()}-tile`
        );
...
describe('weekArr()', () => {
  it('returns array of expected dates', () => {
    const weekStart = DateTime.fromJSDate(
      new Date('1986-10-13T01:43:00')
    ).startOf('week');
    expect(weekArr(weekStart)).toHaveLength(7);
    let week = weekArr(
      DateTime.fromJSDate(new Date('1986-10-13T01:43:00')).startOf('week')
    );
    for (let i = 0; i < 7; i++) {
