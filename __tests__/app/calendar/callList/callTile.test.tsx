import CallTile, {
  CallTileProps,
} from '../../../../app/calendar/callList/callTile';
...
describe('<CallTile />', () => {
  ...
  it('ensemble name is in the document', () => {
    const ensembleName = screen.getByText(
      mockProps.eventCall.event.ensembleName
    );
    expect(ensembleName).toBeInTheDocument();
  });
  it('call start time is in the document', () => {
    const formattedTime = DateTime.fromJSDate(
      new Date(mockProps.eventCall.startTime)
    ).toFormat('hh:mm a'); //hour in 12-hour time, padded to 2:minute, padded to 2 meridiem
