...
import NotificationDetail, { NotificationDetailProps} from "../../components/notifications/detail"
...
describe("<NotificationDetail />", () => {
  ...
  it("notification-detail is in the document", () => {
    const notificationDetail = screen.getByTestId(`${mockProps.playerCall.id}-detail`)
  ...
  it("event-info is in the document with player specific calls", () => {
    ...
    for (let i = 0; i < mockProps.playerCall.calls.length; i ++) {
      const formattedStartTime = String(DateTime.fromJSDate(new Date(mockProps.playerCall.calls[i].startTime)).toFormat("HH:mm DD"))
      const formattedEndTime = String(DateTime.fromJSDate(new Date(mockProps.playerCall.calls[i].endTime)).toFormat("HH:mm DD"))
  ...
  it("additional player message is in the document with message", () => {
    ...
    expect(playerMessage.textContent).toMatch(`${mockProps.playerCall.eventSection.event.fixerName} adds:`)
    expect(playerMessage.textContent).toMatch(mockProps.playerCall.playerMessage)
  })
  it("states who offers gig", () => {
    ...
    const offerOrAvailability = mockProps.playerCall.bookingOrAvailability === "Booking" ? "offers" : "checks availability for"
    const senderInfo = screen.getByText(`${fixer} (${ensembleName}) ${offerOrAvailability}:`)
