...
describe('<NotificationPreview />', () => {
  ...
  it('states ensemble name', () => {
    const ensembleName = screen.getByText(
      mockProps.playerCall.eventSection.event.ensemble.name
    );
    expect(ensembleName).toBeInTheDocument();
  });
  it('states playerCall sent date', () => {
    const recievedDate = screen.getByText(
      DateTime.fromJSDate(mockProps.playerCall.recievedDate).toFormat(
        'HH:mm DD'
      )
    );
  ...
  it('states event title', () => {
    const eventTitle = screen.getByText(
      mockProps.playerCall.eventSection.event.eventTitle
    );
    expect(eventTitle).toBeInTheDocument();
  });
  it('states author of message', () => {
    const fixerName = screen.getByText(
      `${mockProps.playerCall.eventSection.event.fixer.firstName} ${mockProps.playerCall.eventSection.event.fixer.lastName}`
    );
    expect(fixerName).toBeInTheDocument();
  });
  it('states event dates', () => {
    const sortedCalls = mockProps.playerCall.calls.sort(
      (a, b) =>
    ...
    if (
      DateTime.fromJSDate(startDate.startTime).hasSame(
        DateTime.fromJSDate(endDate.endTime),
        'day'
      )
    ) {
      const dateString = DateTime.fromJSDate(startDate.startTime).toFormat(
        'dd LLL yyyy'
      );
