  ...
  it('if filter === action required, all messages require action', () => {
    ...
    for (let i = 0; i < mockProps.playerCalls.length; i++) {
      if (mockProps.playerCalls[i].accepted === null) {
        const playerCallPreview = screen.getByTestId(
          `${mockProps.playerCalls[i].id}-preview`
        );
        expect(playerCallPreview).toBeInTheDocument();
      } else {
        expect(notificationsList.textContent).not.toMatch(
          mockProps.playerCalls[i].eventSection.event.eventTitle
        );
      }
    }
  });
...
describe('<NotificationsList />', () => {
  ...
  it('if filter === past, all messages are from past projects', () => {
    ...
    for (let i = 0; i < mockProps.playerCalls.length; i++) {
      const sortedCalls = mockProps.playerCalls[i].calls.sort(
        (a, b) =>
...
describe('<NotificationsList />', () => {
  ...
  it('if filter === [ensemble], all offers are from selected ensemble', () => {
    ...
    for (let i = 0; i < mockProps.playerCalls.length; i++) {
      if (
        mockProps.playerCalls[i].eventSection.event.ensembleId ===
        mockProps.ensembleFilter
      ) {
        const playerCallPreview = screen.getByTestId(
          `${mockProps.playerCalls[i].id}-preview`
        );
